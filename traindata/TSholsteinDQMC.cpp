...
void LocalUpdates(int &total, int &accept, double &errG, int &lcount, int m, int &L, int &N, double &g, double &dt, double &w, std::uniform_real_distribution<double> &unif, mt19937_64 &rng, double *X, Dmatrix &G, double *B, int &lcountmax, int &lcountb12, double &avgerr, unsigned int &avgerrcounter, int b, int &localoutputcounter){
	...
	for(l=0;l<L;l++){
		...
		for(yy=0;yy<N;yy++){
			for(xx=0;xx<N;xx++){
				...
				//write out data
				if(b==1 && unif(rng)<4./N/N/L){
					std::ofstream file;
					file.precision(15);
					file.open("localtrain.dat", std::ios_base::app);
					file << localoutputcounter << " ";
					file << 0 <<" ";
					file << -1./L/dt*(-dS*dt+2*log(opip)) << " ";
					file << 0 <<" ";
					file << dx <<" ";
					file << X[l*N2+yy*N+xx]<<" ";
					file << X[l*N2+yy*N+(xx+1)%N]<<" ";
					file << X[l*N2+yy*N+(xx-1+N)%N]<<" ";
					file << X[l*N2+((yy+1)%N)*N+xx]<<" ";
					file << X[l*N2+((yy-1+N)%N)*N+xx]<<" ";
					file << X[l*N2+((yy+1)%N)*N+(xx+1)%N]<<" ";
					file << X[l*N2+((yy+1)%N)*N+(xx-1+N)%N]<<" ";
					file << X[l*N2+((yy-1+N)%N)*N+(xx+1)%N]<<" ";
					file << X[l*N2+((yy-1+N)%N)*N+(xx-1+N)%N]<<" ";
					file << X[((l+1)%L)*N2+yy*N+xx]<<" ";
					file << X[((l+1)%L)*N2+yy*N+(xx+1)%N]<<" ";
					file << X[((l+1)%L)*N2+yy*N+(xx-1+N)%N]<<" ";
					file << X[((l+1)%L)*N2+((yy+1)%N)*N+xx]<<" ";
					file << X[((l+1)%L)*N2+((yy-1+N)%N)*N+xx]<<" ";
					file << X[((l+1)%L)*N2+((yy+1)%N)*N+(xx+1)%N]<<" ";
					file << X[((l+1)%L)*N2+((yy+1)%N)*N+(xx-1+N)%N]<<" ";
					file << X[((l+1)%L)*N2+((yy-1+N)%N)*N+(xx+1)%N]<<" ";
					file << X[((l+1)%L)*N2+((yy-1+N)%N)*N+(xx-1+N)%N]<<" ";
					file << X[((l-1+L)%L)*N2+yy*N+xx]<<" ";
					file << X[((l-1+L)%L)*N2+yy*N+(xx+1)%N]<<" ";
					file << X[((l-1+L)%L)*N2+yy*N+(xx-1+N)%N]<<" ";
					file << X[((l-1+L)%L)*N2+((yy+1)%N)*N+xx]<<" ";
					file << X[((l-1+L)%L)*N2+((yy-1+N)%N)*N+xx]<<" ";
					file << X[((l-1+L)%L)*N2+((yy+1)%N)*N+(xx+1)%N]<<" ";
					file << X[((l-1+L)%L)*N2+((yy+1)%N)*N+(xx-1+N)%N]<<" ";
					file << X[((l-1+L)%L)*N2+((yy-1+N)%N)*N+(xx+1)%N]<<" ";
					file << X[((l-1+L)%L)*N2+((yy-1+N)%N)*N+(xx-1+N)%N]<<" ";
					file << endl;
					localoutputcounter++;
				}
...
void GlobalUpdates(int &gtotal, int &gaccept, int &count, int lcount, int &m, int &L, int &N, double &g, double &dt, double &w, std::uniform_real_distribution<double> &unif, mt19937_64 &rng, double *X, Dmatrix &G, double *B,int b, int &globalsamplecounter){
	...
	for(i=0;i<count;i++){
		...
		double dx=(unif(rng)-0.5)*2*g/w/w;
...
//		dS*=g;
		...
		//write out data
		if(b==1 && unif(rng)<4./N/N){
			std::ofstream file;
			file.precision(15);
			file.open("globaltrain.dat", std::ios_base::app);
			file << globalsamplecounter << " ";
			file << -1./L/dt*(-dS*dt+2.*log(numdet)-2.*log(dendet)) << " ";
			for(int l=0;l<L;l++){
				file << X[l*N2+((sitey+1)%N)*N+(sitex-1+N)%N]<<" ";
			}
			for(int l=0;l<L;l++){
				file << X[l*N2+((sitey)%N)*N+(sitex-1+N)%N]<<" ";
			}
			for(int l=0;l<L;l++){
				file << X[l*N2+((sitey-1+N)%N)*N+(sitex-1+N)%N]<<" ";
			}
			for(int l=0;l<L;l++){
				file << X[l*N2+((sitey+1)%N)*N+(sitex)%N]<<" ";
			}
			for(int l=0;l<L;l++){
				file << X[l*N2+((sitey)%N)*N+(sitex)%N]<<" ";
			}
			for(int l=0;l<L;l++){
				file << X[l*N2+((sitey-1+N)%N)*N+(sitex)%N]<<" ";
			}
			for(int l=0;l<L;l++){
				file << X[l*N2+((sitey+1)%N)*N+(sitex+1)%N]<<" ";
			}
			for(int l=0;l<L;l++){
				file << X[l*N2+((sitey)%N)*N+(sitex+1)%N]<<" ";
			}
			for(int l=0;l<L;l++){
				file << X[l*N2+((sitey-1+N)%N)*N+(sitex+1)%N]<<" ";
			}
			file << dx <<" ";
			file << endl;
			globalsamplecounter++;
...
//		X[l]=-1;
	...
	for(int iter=0;iter<warmup;iter++){
		//Local updates
		LocalUpdates(total, accept, errG, lcount, m, L, N, g, dT, w, unif, rng, X, G, B, lcountmax, lcountb12, avgerr, avgerrcounter,0,localoutputcounter);
		//Global updates
		GlobalUpdates(gtotal, gaccept, count, lcount, m, L, N, g, dT, w, unif, rng, X, G, B,0,globalsamplecounter);
		cout<<iter<<endl;
	}
	...
	//Local updates
		LocalUpdates(total, accept, errG, lcount, m, L, N, g, dT, w, unif, rng, X, G, B, lcountmax, lcountb12, avgerr, avgerrcounter,1,localoutputcounter);
		cout<<iter<<endl;
		//Global updates
		GlobalUpdates(gtotal, gaccept, count, lcount, m, L, N, g, dT, w, unif, rng, X, G, B,1,globalsamplecounter);
		//Make Measurements
		SCDWc=0;
		SOc=0;
		for(y1=0;y1<N;y1++){
